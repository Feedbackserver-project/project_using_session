...
<%-- Access faculty information from session --%>
<%-- Assuming the list is stored as "facultyInfoList" in the session --%>
...
<head>
  ...
  function updateFacultyInfo() {
    // Access selected faculty ID from another element (replace "fid" with the actual ID)
    var selectedFacultyId = document.getElementById("fid").value;
    var selectedFaculty = null;

    // Find the selected faculty object based on ID
    for (var i = 0; i < facultyList.length; i++) {
      if (facultyList[i].getFid() === parseInt(selectedFacultyId)) {
    ...
    if (selectedFaculty) {
      ...
      console.log(selectedFaculty.getFname());
      console.log(selectedFaculty.getFid());
      console.log(selectedFaculty.getSub());
      ...
      // Handle case where faculty object is not found (optional)
      console.error("Faculty object not found for ID:", selectedFacultyId);
    }
  }
</script>
</head>
...
<h4>Academic Year: <%= (Integer) session.getAttribute("acc_year") %> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Cycle: <c:out value=<%=session.getAttribute("cycle") %> /></h4>

  <h4>
    Faculty Name: <span id="fname"></span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
    Faculty Id: <span id="fid1"></span>
  </h4>
  <h5>
    Subject: <span id="sub"><%= %></span> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<br>
  </h5>


<h5>Section: <c:out value="${requestScope.sec}" /></h5>
